...
class Timber(object):
    ...
    def czaplewski(DBH, Total_Height, Stem_Height, **kwargs):
        ...
        return DBH * math.sqrt((kwargs['c'] * (Z - 1)) + (kwargs['d'] * (Z2 - 1)) + (kwargs['e'] * ((kwargs['a'] - Z) ** 2) * I1) + (kwargs['f'] * ((kwargs['b'] - Z) ** 2) * I2))
    ...
    def get_merch_dib_from_form(self, Form_Percent = 40, Form_Height = 17):
    ...
    def log_data_to_dict(self, Merch_DIB, Preferred_Log_Length, Minimum_Log_Length):
        # Runs 
        ...
        for (count, (stem_height, dib, length, grade, board_feet)) in enumerate(zip(stem_height_list, dib_list, length_list, grade_list, board_feet_list)):
            temp_dict = {}
            temp_dict['Stem Height'] = stem_height
            temp_dict['DIB'] = dib
            temp_dict['Log Length'] = length
            temp_dict['Log Grade'] = (grade, self.GRADE_NAMES[grade])
            temp_dict['Log Board Feet'] = board_feet
            temp_dict['Log Cubic Feet'] = board_feet / 12
    ...
    def tree_to_dict(self, Merch_DIB, Preferred_Log_Length, Minimum_Log_Length):
        ...
        bf = sum(board_feet_list)

        master = {}
        
        master['Species'] = self.SPP
        master['DBH'] = self.DBH
        master['Merch DIB'] = int(Merch_DIB)
        master['Total Height'] = self.HGT
        master['Merch Height'] = merch_height       
        master['HDR'] = self.HDR
        master['BA'] = self.BA
        master['RD'] = self.RD
        master['Board Feet'] = bf
        master['Cubic Feet'] = bf / 12
        master['VBAR'] = bf / self.BA

        return master


    def tree_per_acre_to_dict(self, Merch_DIB, Preferred_Log_Length, Minimum_Log_Length, Plot_Factor):
        merch_height = self.calc_merch_height(int(Merch_DIB))
        stem_height_list = self.stem_heights(merch_height, Preferred_Log_Length, Minimum_Log_Length)        
        dib_list = [self.get_any_dib(stem_height_list[i]) for i in range(1, len(stem_height_list))]        
        length_list = [self.calc_log_length(stem_height_list[i-1], stem_height_list[i]) for i in range(1, len(stem_height_list))]
        ...
        master['TPA'] = tpa
        master['BA per Acre'] = self.get_BA_acre(Plot_Factor)
        master['RD per Acre'] = self.get_RD_acre(Plot_Factor)
        master['Board Feet per Acre'] = bf * tpa
        master['Cubic Feet per Acre'] = (bf / 12) * tpa
